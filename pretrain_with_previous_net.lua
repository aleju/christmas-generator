...
function main()
    ...
    G = MODELS.create_G(IMG_DIMENSIONS, OPT.noiseDim)

    if OPT.gpu then
        D = NN_UTILS.activateCuda(D)
        G = NN_UTILS.activateCuda(G)
    end
    ...
    local prevNoiseTensor = torch.Tensor(OPT.batchSize * nbBatches, prevNoiseDim)
    local noiseTensor = torch.Tensor(OPT.batchSize * nbBatches, OPT.noiseDim)
    ...
    for i=1,nbBatches do
        ...
        optim.adagrad(fevalD, PARAMETERS_D, OPTSTATE.adam.D)
